...
const Pagination = props => {
    ...
    const pages = getPages().map(pageNum => {
        ...
        return (<li key={pageNum} className={buttonClass} onClick={() => {onPage(pageNum)}}><button className="page-link" >{pageNum}</button></li>);
    });
    ...
    const prevButton = (<li className={prevButtonClass}>
        <button
            className="page-link" onClick={onPrev} tabIndex="-1">Previous</button>
    </li>);
